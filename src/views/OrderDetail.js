...
function OrderDetail() {
      ...
      axios
        ...
        .then((res) => {
          ...
          try {
            ...
            .filter((val, index) => {
              return (val.readStatus==null || (val.readStatus.indexOf(wallet)< 0));
            })
            .map((val, index) => {
              return { readStatus: `${val.readStatus},${wallet}`, id: val.id };
            });
  ...
  const loadHistory = () => {
    ...
    return orderDetail.map((detail, index) => {
      ...
      return <HistoryCard key={index} text={text} date={detail.createdAt} />;
    });
  };
    ...
    <div className="container-xl p-5">
      <div className="card rounded-3 shadow-sm p-4">
            ...
            <div className="mx-3 d-flex flex-column justify-content-between">
              ...
              <div>
                ...
                <span className="mx-4">
                  {" "}
                  Price :{" "}
                  <span className="d-inline-block">{detail.price} </span>
        ...
        <div className="my-3">
          ...
          {!order.orders_details && <div className="d-flex flex-row justify-content-center"><Spinner animation="border" variant="primary" size="lg"/></div>}
          {order.orders_details && loadHistory()}
        </div>
        {order.status == "Waiting" && order.seller_id == session.id && (
          <div
        ...
        {order.status == "Confirmation" && order.buyer_id == session.id && (
          <div
            className="d-flex flex-row align-self-end btn btn-primary"
